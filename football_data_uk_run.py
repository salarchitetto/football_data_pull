...
from utilities.utilities import generate_create_table_query, highlight_text, TextColor
    ...
    for country, data in football_data_uk.items():
        for league in data.get("leagues"):
            ...
            for path, formatted_season in zip(paths_to_process, season_dates.get_formatted_seasons_list()):
                logger.info(f"Running process for season {formatted_season} - {league.name}")

                process_start_time = datetime.now()

                dataframe = dataframe_cleanser(SourceType.EXCEL, path, league, formatted_season)
                hwm = HighWaterMarkProcessor(dataframe, "results", league, formatted_season)

                dataframe_diff = hwm.get_filtered_dataframe()

                # check if the dataframe is empty via HWM
                if dataframe_diff.is_empty():
                    logger.info(f"Incoming data is empty relative to HWM, proceeding to next iteration.")
                    continue

                processed_dataframe = TeamUUIDGenerator(dataframe_diff,
                                                        "home_team", "away_team").uuid_processor()

                FootyPostgres().post_dataframe(processed_dataframe, "results")

                process_end_time = datetime.now()
                process_duration_seconds = highlight_text((process_end_time - process_start_time).total_seconds(),
                                                          TextColor.RED)

                number_of_records_processed = highlight_text(len(processed_dataframe), TextColor.RED)

                logger.info(f"Inserted {number_of_records_processed} records into the results table")
                logger.info(f"Data written for {country.name} - {league.name} | {formatted_season}")
                logger.info(
                    f"Processing for {league.name} | {formatted_season} took: {process_duration_seconds} seconds")
                logger.info(f"{'*' * 100}")
                logger.info("")
...
if __name__ == "__main__":
    ...
    full_duration_seconds = highlight_text((full_end_time - full_start_time).total_seconds(),
                                           TextColor.RED)
