...
class ProcessorBackFill:
    ...
    def process_back_fill(self, id_generator: TeamIDGenerator) -> None:
        ...
        zipped_paths_dates = zip(download_paths, formatted_season_list)

        dataframes = dataframe_util.create_dataframe_list(zipped_paths_dates)
        columns = column_util.find_all_column_names(dataframes)
        added_cols_dataframe = column_util.add_missing_columns_to_dataframe(columns, dataframes)

        # TODO: refactor this
        if self.configs.file_name == LeagueDictionary.PREMIER_LEAGUE.name.lower():
            dataframe = dataframe_util.union_dataframes(added_cols_dataframe)
            ...
            for dataframe in added_cols_dataframe:
                dataframe = dataframe_util.clean_dataframe(dataframe, self.configs.league_name)
